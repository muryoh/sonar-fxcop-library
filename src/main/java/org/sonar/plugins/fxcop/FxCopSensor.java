...
public class FxCopSensor implements Sensor {
  ...
  void analyse(SensorContext context, FileProvider fileProvider, FxCopRulesetWriter writer, FxCopReportParser parser, FxCopExecutor executor) {
    ...
    executor.execute(settings.getString(fxCopConf.fxCopCmdPropertyKey()),
        settings.getString(fxCopConf.assemblyPropertyKey()), rulesetFile,
        reportFile, settings.getInt(fxCopConf.timeoutPropertyKey()),
        settings.getString(fxCopConf.directoryPropertyKey()), settings.getBoolean(fxCopConf.aspnetPropertyKey()));
